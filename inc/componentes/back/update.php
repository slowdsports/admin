...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    elseif (isset($_POST['fuentes'])) {
        ...
        $pais = $_POST["pais"];
        $tipo = $_POST["tipo"];
        $fComentario = $_POST["comentario"];
        $sql = "UPDATE `fuentes` SET `fuenteId`='$id', `fuenteNombre`='$fNombre', `canal`='$cPadre', `canalUrl`='$fUrl', `key`='$key1', `key2`='$key2', `pais`='$pais', `tipo`='$tipo', `status`=$fStatus, `comentario`='$fComentario' WHERE fuenteId=$id";
        if (mysqli_query($conn, $sql)) {
